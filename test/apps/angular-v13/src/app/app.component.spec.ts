...
describe('Unit Tests', () => {
  ...
  beforeEach(() => {
    ...
    if (process.env['ninja_TESTING_DISABLEHTTPSCHECK']) {
      testing = {
    ...
    TestBed.configureTestingModule({
      imports: [
        ...
        ninjaAuthModule.forRoot({ninjaAuth, testing})
      ],
      declarations: [ 
        AppComponent
      ],
  ...
  it('can retrieve an accessToken and idToken from the tokenManager', async () => {
    ...
    const accessToken = await component.ninjaAuth.getAccessToken();
    expect(accessToken).toBe(mockAccessToken);
    const idToken = await component.ninjaAuth.getIdToken();
  ...
  it('isAuthenticated() returns true when the TokenManager returns an access token and id token', async () => {
    ...
    const authenticated = await component.ninjaAuth.isAuthenticated();
