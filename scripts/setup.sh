#!/bin/bash -xe

DIR=$(cd -- "$( dirname -- "${BASH_SOURCE[0]}" )" &> /dev/null && pwd)
source $DIR/utils/local.sh
source $DIR/utils/siw-platform-scripts.sh

# Can be used to run a canary build against a beta AuthJS version that has been published to artifactory.
# This is available from the "downstream artifact" menu on any ninja-auth-js build in Bacon.
# DO NOT MERGE ANY CHANGES TO THIS LINE!!
export AUTHJS_VERSION=""

# Install a specific version of auth-js, used by downstream artifact builds
install_auth_js () {
  if [ ! -z "$AUTHJS_VERSION" ]; then
    echo "Installing AUTHJS_VERSION: ${AUTHJS_VERSION}"
    install_siw_platform_scripts
    install_artifact @ninja/ninja-auth-js "${AUTHJS_VERSION}"
    echo "AUTHJS_VERSION installed: ${AUTHJS_VERSION}"

    # verify single version of auth-js is installed
    # NOTE: ninja-signin-widget will install it's own version of auth-js, filtered out
    AUTHJS_INSTALLS=$(find . -type d -path "*/node_modules/@ninja/ninja-auth-js" -not -path "*/ninja-signin-widget/*" -not -path "*/test/apps/angular-*/*"  | wc -l)
    if [ $AUTHJS_INSTALLS -gt 1 ]; then
      echo "ADDITIONAL AUTH JS INSTALL DETECTED"
      yarn why @ninja/ninja-auth-js
      exit ${FAILED_SETUP}
    fi
  fi
}

# Install required node version
export NVM_DIR="/root/.nvm"
setup_service node v16.14.0

# Install yarn
# Use the cacert bundled with centos as ninja root CA is self-signed and cause issues downloading from yarn
setup_service yarn 1.21.1 /etc/pki/tls/certs/ca-bundle.crt

# Add yarn to the $PATH so npm cli commands do not fail
export PATH="${PATH}:$(yarn global bin)"

cd ${ninja_HOME}/${REPO}

# if running on bacon
if [ -n "${TEST_SUITE_ID}" ]; then
  # undo permissions change on scripts/publish.sh
  git checkout -- scripts

  # ensure we're in a branch on the correct sha
  git checkout $BRANCH
  git reset --hard $SHA

  git config --global user.email "ninjauploader@ninja.com"
  git config --global user.name "ninjauploader-ninja"
fi

# Install dependencies but do not build
if ! yarn install --frozen-lockfile --ignore-scripts; then
  echo "yarn install failed! Exiting..."
  exit ${FAILED_SETUP}
fi

# Install a specific version of auth-jss
install_auth_js

# Build
if ! yarn build; then
  echo "yarn build failed! Exiting..."
  exit ${FAILED_SETUP}
fi
