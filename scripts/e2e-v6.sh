...
source ${ninja_HOME}/${REPO}/scripts/setup-e2e.sh
...
export ISSUER=https://samples-javascript.ninja.com/oauth2/default
...
if ! yarn add -DW --ignore-scripts @ninja/ninja-auth-js@^6; then
...
do
  pushd $app
    if ! yarn add --ignore-scripts @ninja/ninja-auth-js@^6; then
